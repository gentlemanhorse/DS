...
void entMatPonteiro(int** mat, int lin, int col){
    for(int i = 0; i < lin; i++){
        for(int j = 0; j < col; j++){
            printf("Entre o elemento [%d][%d] da matriz\n>>>", i + 1, j + 1);
            scanf("%d", &mat[i][j]);
        }
    }
}
...
int** multiplicaMatrizes(int** mat1, int* mat2, int lin1, int col1, int lin2, int col2){
...
int main(int arc, char* argv[]){
    ...
    if(col1 == lin2){
        ...
        int** matF = multipicaMatrizes(matS, matP, lin1, col1, lin2, col2);
